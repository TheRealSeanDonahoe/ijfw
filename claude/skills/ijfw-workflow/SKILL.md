...
AI lists 5 things we could build but won't. User confirms or pulls one back in. Donahoe P4.
...
- Dispatch-planner for parallelism: `ijfw-dispatch-plan <plan.md>` surfaces the manifest one-liner before any sub-wave dispatch.
...
- Functional + UX + Security + Quality checklists from Donahoe 22 (see references/donahoe-principles.md).
...
No `Phase 10 / Wave 10A` hardcoded numbers. Narration tracks the current workflow step, not historical plan-doc coordinates.
...
- `references/donahoe-principles.md` -- 22 principles used in VERIFY + SHIP audits.
- `.planning/phase13/BRAINSTORM-RESEARCH.md` -- framework research that shaped the Quick + Deep flows.
