...
| D04 [closed] | `mcp-server/src/cross-orchestrator-cli.js:394` | LOW | On a successful cross run the CLI prints `Receipt: .ijfw/receipts/cross-runs.jsonl` -- an internal dot-file path exposed at peak success moment. Hook scripts explicitly ban JSONL / file paths in user output (session-start.sh:15). Inconsistent discipline between CLI output and hook output. | Replace with `Receipt logged -- run \`ijfw status\` to see it` or similar action phrase. | Closed in P11 Wave 11A-mcp. |
