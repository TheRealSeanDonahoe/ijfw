...
|-----|--------------------------------------------------------------|--------|---------------|-----------------|---------------|
| C01 | mcp-server/src/cross-orchestrator-cli.js:313                 | K,D    | HIGH          | Doctor row `[ -- ] <id> CLI -- not found` is literal negative framing; violates sacred positive-framing invariant and is first diagnostic stranger sees. | Replace with `[ .. ] <id> CLI -- install to unlock -- try: brew install <x>`. |
| C02 | mcp-server/src/cross-orchestrator-cli.js:320                 | K,D    | HIGH          | Doctor API-key row `[ -- ] <ENV> -- not set` is same negative framing. | Replace with `[ .. ] <ENV> -- set to enable <feature>` with inline export hint. |
| C03 | mcp-server/bin/ijfw:15                                       | K,D    | HIGH          | Launcher stderr `Error: ijfw CLI not found at $CLI` is negative at the first surface a stranger touches. | Rewrite `IJFW launcher expects CLI at $CLI -- reinstall @ijfw/install to restore it.`. |
| C04 | mcp-server/src/hero-line.js:46                               | K,S,D  | MED           | Empty-state `No cross-audit runs yet` is silent / does not teach the next step. | Rewrite `Trident is warm -- run ijfw cross audit <file> to record the first receipt. First run ~20s.`. |
| C05 | mcp-server/src/hero-line.js:80-112                           | K,D    | HIGH          | Hero line interpolates non-ASCII code points (U+2212, U+0394, U+00D7, middle dot). Violates ASCII-only rule on the highest-value user-facing surface. | Replace: unicode minus -> `-`; delta glyph -> `delta:`; multiplication sign -> `x`; middle dot -> ` -- `. |
| C06 | mcp-server/src/cross-orchestrator-cli.js (cmdDoctor) 297-334 | K,S,D  | HIGH          | Doctor output is commoditised `[ok]/[--]` grid: negative-framed rows (K/D) and no value reframing of what each auditor unlocks (S). | Two-part fix: (a) positive-frame every row per C01/C02; (b) add per-auditor value line and closing "N of 6 auditors reachable -- full Trident available / partial coverage available". |
| C07 | README.md:13 (install one-liner) + README.md:46,55-83         | K,D    | HIGH          | Install one-liner conflates install-time and post-install; `ijfw install <platform>` invocations in README do not exist on the shipped binary. Stranger cannot follow the README as written. | Change install one-liner to `npm install -g @ijfw/install && ijfw-install && ijfw demo` (or add `install` passthrough to `ijfw` CLI); remove or rewire the five per-platform invocations. |
| C08 | README.md:110 (tool count)                                   | K,D    | HIGH          | README states "MCP server (5 tools)" but server exposes 7-8 tools; CLAUDE.md says `<=8`. | Update to accurate count and list all tools, or drop the count and describe as "shared via stdio, zero deps". |
| C09 | installer/src/install.js:107 (preflight header)              | K,D    | HIGH          | `Preflight:` header is neutral/unclear about blast radius; followed by negative bullets like `Node X detected; IJFW wants >=18` and `git not on PATH`. | Change header to `IJFW needs a couple of things first -- fix these and re-run:`; rewrite bullets to `IJFW needs git on PATH -- install git, then retry.` (and peers). |
| C10 | universal/ijfw-rules.md (file-truth + em-dash)               | K,D    | HIGH          | K flags manifest/README/CLAUDE.md disagreement about path and line budget (19 lines vs documented 15). D flags em-dash on line 13 violating ASCII-only. | Pick canonical path, align manifest + README + CLAUDE.md; replace em-dash with ASCII `--`. |
| C11 | gemini/GEMINI.md em-dash + install precondition              | K,D    | HIGH          | D flags em-dashes on lines 19, 23 (ASCII violation). K flags line 38 tells user to run `ijfw cross ...` before install precondition documented. | Replace em-dashes with `--` or `:`; prepend install precondition line. |
| C12 | cursor/.cursorrules:23 em-dash                               | D      | HIGH (single-lens, kept visible for grep consistency with C10/C11) | Same em-dash pattern in cursor rules file. | Same fix as C10/C11. |
...
| K3.1 | HIGH | claude/commands/ijfw.md:18                                  | `/ijfw memory audit` is a dead link; route to `memory-audit.md` or add dispatcher. |
...
| K10.4| MED  | PUBLISH-CHECKLIST.md:66                                     | Ambiguous pass condition for HIGH-finding grep. |
...
| S3.1 | HIGH | claude/commands/ijfw.md:7-32                                | Command index is categorically identical to every other CLI help; add headline value reframe. |
| S4.1 | HIGH | mcp-server/src/cross-orchestrator-cli.js:162-179            | cmdStatus ends with "Total runs: N" -- no cumulative value (hours saved / findings caught). |
| S5.1 | HIGH | mcp-server/src/server.js:464-465                            | Tool desc "Retrieve context from IJFW memory" is what it does, not what user gets. Rewrite benefit-first. |
| S5.2 | HIGH | mcp-server/src/server.js:517                                | `ijfw_memory_status` description reads like docstring; rewrite as "Ready-to-inject project brief (~200 tokens)". |
| S6.1 | HIGH | mcp-server/src/hero-line.js:80-112                          | Hero line is strongest Sutherland asset but buried -- surface in session-start banner, /status, workflow SHIP gate, installer post-run, demo closer. |
| S7.2 | HIGH | claude/hooks/scripts/session-start.sh:310-385               | Wake-up banner composes many signals but lacks cumulative-value line (tokens saved, cache hit rate, findings caught). |
| S8.1 | HIGH | installer/src/install.js:138-143                            | Post-install closer misses receipt: "N platforms configured, M MCP tools registered, existing configs merged not clobbered". |
| S10.1| HIGH | README.md:1-13                                              | Opening missing numeric wow line above install command. |
| S10.3| HIGH | README.md:117-123                                           | Why-IJFW bullets do not mention Trident -- the most distinctive asset is absent. |
...
| ID    | Sev  | Surface                                                         | Action |
|-------|------|-----------------------------------------------------------------|--------|
| D4.5  | HIGH | mcp-server/src/cross-orchestrator-cli.js:332                    | `No auditors reachable yet` leads negative; reframe "IJFW has the Trident ready -- install codex or gemini (or set OPENAI_API_KEY / GEMINI_API_KEY), then run ijfw demo." |
| D4.6  | HIGH | mcp-server/src/cross-orchestrator-cli.js:353-358                | Classic `Error: mode must be one of ...` framing; rewrite actionable. |
| D4.7  | HIGH | mcp-server/src/cross-orchestrator-cli.js:371                    | Raw error bubble-up on run failure; wrap with `Run ijfw doctor to see what to fix.`. |
| D8.1  | HIGH | installer/src/install.js:65                                     | `>=` rendered as Unicode `GREATER-THAN OR EQUAL`; replace with ASCII `>=`. |
| D8.2  | HIGH | installer/src/install.js:66-68                                  | `git not on PATH` / `bash not on PATH` banned negatives; rewrite "IJFW needs git on PATH -- install git, then retry." |
| D8.3  | HIGH | installer/src/install.js:86,91,100                              | Raw `git pull failed` / `git clone failed` / `scripts/install.sh exited N` bubbles to user stderr. Wrap user layer. |
| D8.4  | HIGH | installer/src/install.js:97                                     | `scripts/install.sh missing at X.` banned "missing"; rewrite. |
| D8.6  | HIGH | scripts/install.sh:42                                           | `Launcher missing at X -- aborting.` two banned words one line; rewrite. |
