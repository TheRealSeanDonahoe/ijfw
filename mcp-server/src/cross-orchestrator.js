...
// Spawn a single external auditor. Returns raw stdout string or null on error.
function fireExternal(pick, request, timeoutMs = 600_000) {
  return new Promise((resolve) => {
    ...
    proc.on('close', () => { clearTimeout(timer); resolve(stdout || null); });
...
export async function runCrossOp({
  ...
  const receipt = {
    ...
    mode,
    target,
    auditors: picks.map(p => ({ id: p.id, family: p.family, model: p.model || '' })),
    findings,
    duration_ms,
    input_tokens: null,
    cost_usd: null,
